"""
Build the final cleaned and concatenated TRACE dataset. Loads all necessary functions specified in the data management
files to merge the reading-in and data cleeaning steps so as to produce the final dataset.
Define all final dataset specifications in the dataset_specs dictionary.
"""
...
# Import function to concatenate yearly data and merge the rating, issue and bond info data to the transaction data
...
# Import the inter-dealer transaction and agency trade filter according to Dick-Nielsen & Poulsen (2019)
...
# Import the function to assign the regulatory period indicator and map the ratings to the Basl risk weights
...
dataset_specs = {
    ...
    'transactions': {
        'varlist': ['CUSIP_ID', 'RPTG_PARTY_ID','RPT_SIDE_CD','BUY_CPCTY_CD','SELL_CPCTY_CD',
                   'CNTRA_PARTY_ID', 'ENTRD_VOL_QT', 'RPTD_PR', 'TRD_RPT_DT', 'TRD_EXCTN_DT', 'TRD_EXCTN_TM',
                   'CMSN_TRD', 'TRDG_MKT_CD']
    },
    # Specify the variables to keep from the issue data
    'issue_data': {
        'varlist': ['CUSIP_ID', 'maturity', 'offering_amt', 'offering_price', 'principal_amt', 'bond_type',
                   'amount_outstanding', 'effective_date', 'putable', 'offering_date', 'active_issue']
    },
    # Specify the variables to keep from the bond background information data
    'bond_info': {
        'varlist': ['CUSIP_ID', 'RULE_144A_FL']
    },
    # Specify the variables to be kept in the final DataFrame of the cleaning step (All variables here need to be in
    # the union of the variables specified above)
    'dataset_clean': {
        'varlist': ['CUSIP_ID', 'RPTG_PARTY_ID', 'CNTRA_PARTY_ID', 'ENTRD_VOL_QT', 'RPTD_PR', 'RPT_SIDE_CD',
                    'TRD_EXCTN_DT', 'TRD_EXCTN_TM', 'CMSN_TRD', 'TRDG_MKT_CD', 'BUY_CPCTY_CD', 'SELL_CPCTY_CD', 'maturity',
                    'rating_date', 'rating', 'rating_numeric', 'principal_amt', 'offering_date', 'offering_amt', 'offering_price',
                    'amount_outstanding', 'effective_date', 'active_issue']
    }
}

# 0.3) Set up file paths to automatically check if specific steps have already been executed and hence can be skiped
...
path_TRACE_raw_clean_first = f"{os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))}/bld/data/TRACE/TRACE_raw_clean/{TRACE_first_df}"
path_TRACE_raw_clean_last = f"{os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))}/bld/data/TRACE/TRACE_raw_clean/{TRACE_last_df}"
path_TRACE_bond_info = f"{os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))}/bld/data/TRACE/TRACE_raw_clean/bond_info.pkl"
df_all_rpt_dates = f"{os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))}/bld/data/TRACE/TRACE_raw_clean/TRACE_info/TRACE_rpt_dates.pkl"

print("START TO READ AND CLEAN THE ACADEMIC TRACE DATA FOR THE TIME RANGE: {} to {}".format(dataset_specs['sample_time_span'][0], dataset_specs['sample_time_span'][1]))
...
# 1.2) Read in the daily bond background characteristics
if (os.path.isfile(path_TRACE_bond_info)):
    print("")
...
######
# 3) Construct default input variables
## 3.1) Add trad size, etc.
######
